...
const rootCA = require('./byteAnalysisRCA').rootCA();
...
//	}];
	...
	// Root CA private key and cert is used to sign the
	// newly generated Domain Certificate
	const rootCAKey = pki.privateKeyFromPem(rootCA.privateKey);
	const rootCACert = pki.certificateFromPem(rootCA.ca);
	...
	newCert.setIssuer(rootCACert.subject.attributes);
	newCert.publicKey = csr.publicKey; // this same public key is used for all domains
	...
	// Chrome did not work, probably X509 version 3 needs this to be intact
	// for browsers to work?
	...
	newCert.sign(rootCAKey, md.sha256.create());

	const domainCert = pki.certificateToPem(newCert);
	const domainKey = pki.privateKeyToPem(keys.privateKey); // same private key is used for all domains
