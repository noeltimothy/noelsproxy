...
responseHandlers.push (function (req, conn) {
	req.pipe(conn);
});
...
const callback = function(response) {
	...
	//console.log ('sending actual req to ' + options.host);
 	var agentRequestor = req.isSSL? https.request(options, callback) : http.request(options, callback)
	var responseStr = [];
	...
	//serverObj.emit('onResponse', req, res, agentRequestor)
})
...
function noelsproxy (httpPort, httpsPort) {
	var self = this;
...
noelsproxy.prototype.__proto__ = events.EventEmitter.prototype;
module.exports = noelsproxy
